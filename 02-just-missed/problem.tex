...
\begin{document}
	...
	For example, if $n=5$, the first $5$ primes are $2,3,5,7,11$. This means that $k\in[10,\infinity)$.
	\begin{itemise}
		...
		\item $14\pmod11=3$, move on to the next number
		\item $15\pmod11=4$, move on to the next number
		...
		\item $20\pmod11=9$, move on to the next number
		\item $21$:
		\begin{itemise}
			\item $21\pmod2=1$
			\item $21\pmod3=0$
	...
	\begin{tabular}{ |l|l| } 
		...
		{Sample input} & {Sample output} \\ % use {\lstinputlisting{./testCases/input/input00.txt}} & {\lstinputlisting{./testCases/output/output00.txt}}
		\hline
	\end{tabular}
